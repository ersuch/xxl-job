...
--
-- 示例：创建数据库和用户（按需执行）
-- 1. 使用 postgres 超级管理员登录后执行以下命令
--
--     -- 创建数据库
--     CREATE DATABASE xxl_job;
--
--     -- 创建用户并设置密码
--     CREATE USER xxl_job_user WITH PASSWORD 'xxl_job_password';
--
--     -- 授予数据库权限
--     GRANT ALL PRIVILEGES ON DATABASE xxl_job TO xxl_job_user;
--
--     -- 切换到 xxl_job 数据库后，授予 Schema 权限（PostgreSQL 默认 schema 为 public）
--     GRANT ALL PRIVILEGES ON SCHEMA public TO xxl_job_user;
--     GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO xxl_job_user;
--     GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO xxl_job_user;
--
-- 2. 也可直接使用 superuser（postgres），application.properties 配置如下：
...
CREATE TABLE xxl_job_registry (
    ...
    update_time       TIMESTAMP,
    CONSTRAINT uk_registry_group_key_value UNIQUE (registry_group, registry_key, registry_value)
);
...
COMMENT ON TABLE xxl_job_user IS '用户信息表';
...
VALUES (1, '示例任务01', NOW(), NOW(), 'XXL', '', 'CRON', '0 0 0 * * ? *',
    ...
    "apiKey": "app-OUVgNUOQRIMokfmuJvBJoUTN"
        ...
        NOW(), ''),
       (2, 'OpenClaw示例任务', NOW(), NOW(), 'XXL', '', 'NONE', '',
